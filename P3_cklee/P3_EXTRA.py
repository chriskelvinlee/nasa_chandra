...
__global__ void sharpeningFilter( float* next_im, float* curr_im, int height, int width, double EPSILON )
{
	...
	if ( i > 0 && i < width-1 && j > 0 && j < height-1 )
	{
	    ...
	    {
    	    next_im[gtid] = s_curr_im[stid]  + EPSILON * (
    		    -1*s_curr_im[stid - blockDim.x-1] + -2*s_curr_im[stid - blockDim.x] + -1*s_curr_im[stid - blockDim.x+1] + 
    		    -2*s_curr_im[stid-1]              + 12*s_curr_im[stid]              + -2*s_curr_im[stid+1] + 
    			-1*s_curr_im[stid + blockDim.x-1] + -2*s_curr_im[stid + blockDim.x] + -1*s_curr_im[stid + blockDim.x+1]);
	    }
...
imsave('Harvard_Sharpened_GPU_EXTRA.png'.format(NUM_ITERATIONS), curr_im, cmap=cm.gray, vmin=0, vmax=1)
...
if calculate_error == True:
    ...
    parallel_file   = "Harvard_Sharpened_GPU_EXTRA.png"     

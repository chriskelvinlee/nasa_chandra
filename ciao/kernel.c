  ...
  for( kk=nks; kk--; ) {
	...
	for ( subpix_jj=subpix;subpix_jj--; ) {
	  ...
	  for ( subpix_ii=subpix;subpix_ii--; ) {
	    ...
	    } else if (*wever == 's' ) { /* sinc */
	      if ( ( 0 == rad ) || ( 0 == dist )) 
		lker[pix] += 1.0*subpix_area;
	      else
		lker[pix] += sin( 6.283184* dist / rad ) / ( 6.283184*dist/rad);
	      /*exp( -0.5 * dist_sq / rad_sq)*subpix_area; */
	    } else if (*wever == 'm' ) { /* mex hat */
	      if ( 0 == rad )
		lker[pix] += 2.0*subpix_area;
	      else
		lker[pix] += (2-(dist_sq/rad_sq))*exp(-0.5*dist_sq/rad_sq)*
		  subpix_area;
	    } else if (*wever == 'l' ) {
	      if (( rad == 0 ) || ( dist < FLT_EPSILON ))
		lker[pix] += 1.0*subpix_area;
	      else
		lker[pix] += subpix_area / (dist_sq/rad_sq);
	    } else if (*wever == 'e' ) {
	      if (( rad == 0 ) || ( dist < FLT_EPSILON ))
		lker[pix] += 1.0*subpix_area;
	      else
		lker[pix] += exp( -dist/rad  ) *subpix_area;
		/* lker[pix] += exp( (-fabs(_ii) -fabs(_jj))/rad ) *subpix_area; */
	    } else if (*wever == 'w' ) {
	      if (( rad == 0 ) || ( dist < FLT_EPSILON ))
		lker[pix] += 1.0*subpix_area;
	      else
		lker[pix] += pow( dist, -1.0/rad ) * subpix_area;
		/* lker[pix] += pow( fabs(_ii), -rad ) * pow(fabs(_jj), -rad) *
		   subpix_area; */
	    } else if ( *wever == 't' ) { /* tophat */
	      if ( dist < rad ) 
		lker[pix] += subpix_area ;
	      else
		lker[pix] += 0;
	    } else if ( *wever == 'b' ) { /* box */
	      lker[pix] += subpix_area;

	    } else if ( *wever == 'c' ) { /* cone */
	      if ( dist < rad ) 
		lker[pix] += (subpix_area*(1-(dist/rad)));
	      else
		lker[pix]+=0;
	    } else if ( *wever == 'q' ) { /* quad */
	      if ( dist < (rad+1) ) 
		lker[pix] += (subpix_area*(1-(dist_sq/((rad+1)*(rad+1)))));
	      else
		lker[pix]+=0;
	    } else if ( *wever == 'h' ) { /* hemisphere */
	      if ( dist < rad ) 
		lker[pix] += sqrt( rad_sq - dist_sq );
	      else
		lker[pix]+=0;
	    } else if ( *wever == 'p' ) { /* pyramid */
	      double mm;
	      if ( fabs(_ii) > fabs(_jj))
		mm = fabs(_ii);
	      else
		mm = fabs(_jj);

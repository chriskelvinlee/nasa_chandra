...
slightly different for the dmgti tool. Note that prior to CIAO 4.3
this list of 'special-case' tools was significantly larger.
...
New in the May 2011 release is limited support for stacks (see "ahelp
stack" for background information). String and file parameters may be set
using an array, in which case the value is automatically converted
into a comma-separated string.

There is no support for converting a parameter value from a stack into
an array; the stack module can be used for this.
...
NOTE: special case for dmgti
----------------------------

As of CIAO 4.3 there is only one tool - dmgti - that does not support
the <@@name.par> syntax (see 'ahelp parameter' for more information on
this functionality). Care should be taken to avoid running multiple
copies of dmgti at the same time; if this is likely to happen use the
set_pfiles() routine to set up individual user parameter directories.
...
As noted above in the 'special case for dmgti' section, this method is
not used for the dmgti tool. You may need to use set_pfiles() to set
up a unique PFILES environment if you are run dmgti multiple times
simultaneously; an alternative is to use the new_pfiles_environment()
context manager to automate the creation of a new PFILES environment
and directory for each run.
...
raises the error
  ...
  IOError: An error occurred while running 'dmcopy':
    ...
    # DMCOPY (CIAO 4.3): DM Parse error: unmatched [ or (: [cols x
...
def _values_equal(ptype, val1, val2):
    ...
    if ptype != "r" or (val1 == None and val2 == None):
        return val1 == val2
...
class CIAOParameter(object):
    ...
    def __init__(self, toolname, reqs, opts):
        ...
        self._store    = dict(zip(self._required, reqs) +
                              zip(self._optional, opts))

        # we include names that are not mapped just to make it easier
        self._orig_parnames = dict(zip(self._required, [p.name for p in reqs]) +
                                   zip(self._optional, [p.name for p in opts]))

        self._defaults = dict([(pname,self._store[pname].default) for pname in self._parnames])
        self._settings = {} # should only be accessed via _set/get_param_value
        self._index = 0
        self.punlearn()
    ...
    def _set_param_value(self, pname, nval):
        ...
        if hasattr(nval, "__iter__"):
            # should really enforce the type constraint for safety here
            self._settings[pname] = ",".join(map(str, nval))
    ...
    def _param_as_string(self, pname):
        ...
        #if pi.type == "f" and pval == None:
        #    pval = "INDEF"

        if pi.type in "sf" and pval == None:
            pval = ""
    ...
    def _validate(self, pname, val, store=True):
        ...
        isiterable = hasattr(pval, "__iter__")
    ...
    def __contains__(self, parname):
        ...
        return (matches != None) and matches[1]
    ...
    def _create_parfile_copy(self, parfile=None):
        ...
        is created. It is up to the caller to make sure that this file
        ...
        if parfile == None:
            tmpfile = True
            pfh = tempfile.NamedTemporaryFile(suffix=".par", delete=False,
                                              mode="w")
            parfile = pfh.name[:]
        ...
        try:
            ...
            for pname in self._parnames:
                ...
                if len(nval) > 1023:
                    ...
                    sfh = tempfile.NamedTemporaryFile(delete=False, mode="w")
                    sname = sfh.name[:]
            ...
            # The paramio module throws Exceptions, so we check to see whether
            # we have a StandardError or below to see what we want to throw
            #
            if isinstance(ee, StandardError):
                raise
            else:
    ...
    def _update_parfile_verify(self, parfile, stackfiles):
        ...
        Note that if an external stack file is used we do not currently validate
        its values (it should be, for safety, but isn't).
        ...
        try:
            ...
            for pname in self._parnames:
                ...
                try:
                    ...
                    v5("parameter {0} has been set to an external stack file".format(pname))
                    ...
                    try:
                        ...
                        v5("skipping verifying parameter {0} as not set".format(pname))
                        continue
            ...
            if isinstance(ee, StandardError):
                ...
                raise IOError("Unable to check values in parameter file:\n  {0}".format(parfile))
    ...
    def _update_parfile(self, parfile):
        ...
        v5("_update_parfile[{0}] called for {1}".format(
                self._toolname, parfile))
        ...
        try:
            ...
            for sname in stackfiles.itervalues():
                try:
    ...
    def write_params(self, parfile=None):
        ...
        if len(stackfiles) == 0:
            ...
            for (k,v) in stackfiles.iteritems():
    ...
    def read_params(self, parfile=None):
        ...
        if pname == self._toolname:
            msg = "Read from the {0} parameter file".format(self._toolname)
        else:
            msg = "Read parameters for {0} from {1}".format(self._toolname, pname)
...
class CIAOTool(CIAOParameter):
	...
	if str(pval).startswith(")"):
	    ...
	    newpar = pval[1:].replace('-','_') # the '-'/'_' replacement is prob. not worth doing
	...
	parnames = self._parnames
        ...
        nstore = {}
        for (pname,pval) in store.iteritems():
            parname = self._expand_parname(pname)
            nstore[parname] = pval
            
        for pname in [pn for pn in self._parnames if pn in nstore]:
            pval = nstore[pname]
            v5("Setting parameter {0} to {1} [from {2}]".format(pname, pval, type(pval)))
    ...
    def get_runtime_details(self):
        ...
        rval = {}
        if self._runtimes != None:
            for (k,v) in self._runtimes.iteritems():
                rval[k] = v
...
class CIAOToolParFile(CIAOTool):
    ...
    def _run(self, parfile):
        ...
        # serr will be None below by construction, since we tell Popen to
        # merge the stderr channel into stdout
        #
        ...
        proc = subprocess.Popen([self._toolname, "@@{0}".format(parfile), "mode=hl"],
...
def set_pfiles(userdir=None, userdirs=None):
    ...
    if userdir == None and userdirs == None:
        newpath = ""
...
def new_tmpdir(dirname=None):
    ...
    used; this may not be ideal if a large amount of data is
    placed in the directory (it depends on where tempfile.mkdtemp
    places the directory).
    ...
    if dirname == None:
        dname = tempfile.mkdtemp(prefix="tmpdir")
...
@contextmanager
def new_pfiles_environment(dirname=None, ardlib=True, copyuser=True):
    """A context manager which creates a new parameter directory, copies
    over the existing ardlib parameter file to it (if ardlib is True),
    and then sets the PFILES env. var to use this directory (for the user
    path). The block of code is executed and then the temporary
    parameter directory is removed and the preceeding PFILES
    setting is restored.
    ...
    and scripts (really this should be added to the system part of the
    path). There is the potential for a race condition here (in that
    if an external process is editing a parameter file whilst it is being
    copied the result may be an invalid parameter file).

    See the new_tmpdir context manager for details of how the
    temporary directory is created (the dirname argument is passed
    through for this).

    The old restriction (from the April 2011 version) that the user
    PFILES section could only contain one directory has been removed;
    the copyuser argument has been added to try and deal with such
    situations.
    ...
    An example of use:
      ...
      # these tools (or changes to the default ardlib oparameter file
    ...
    # possibility at the shell prompt when runn
    #
    ...
    with new_tmpdir(dirname) as dname:
            ...
            if copyuser and odirs != None:
                ...
                for od in odirs[::-1]:
                    ...
                    for pname in glob.glob("{0}/*.par".format(od)):
                        ...
                        if dtime.search(bname) != None:
                            continue
                            
                        bname = bname[:-4]
                        try:
                            # use pio.paramgetpath since this just finds the parameter
                            # file; pio.pacccess will also copy it, which we don't want
                            # here
                            #
                            pio.paramgetpath(bname)
 
                        except Exception:
                            v5("Copying over {0} as not in system path".format(pname))
                            shutil.copy2(pname, dname)
                        
            if ardlib:
                v5("About to copy over ardlib parameter file: {0}".format(ardlibpath))
                shutil.copy2(ardlibpath, dname)
                
            yield dname

        finally:
            #v5("About to restore user PFILES setting to {0}".format(odirs))
            #set_pfiles(userdirs=odirs)
...
def list_tools(tools=True, params=True):
    ...
    out = [pname for (pname,pi) in parinfo.iteritems() if pi["istool"] in allowed]
    out.sort()
    return out

# Those tools which can not be run using <toolname> @@foo.par.
# As of CIAO 4.3 this is a small list!
#
_no_par_file = [ "dmgti" ]
...
parinfo['acis_build_badpix'] = {
    'istool': True,
    'req': [ParValue("pbkfile","f","Parameter block file",None),ParValue("berrfile","f","Bias error files",None),ParValue("calibfile","f","Calibration bad pixel file",None),ParValue("biasfile","f","Bias images",None),ParValue("evt0file","f","L0 Event lists",'none'),ParValue("outfile","f","Output filename",None)],
    'opt': [ParValue("pbkext","s","Parameter block extension",'PBK'),ParValue("berrext","s","Bias error extension",'BERR'),ParValue("maxerr","i","Maximum no. of bias parity errors to record per exposure",10),ParValue("usrfile","f","User-specified supplemental bad-pixel file (NONE -- none -- <filename>)",'none'),ParValue("bitflag","s","A 32-character string where 0=exclude pixel, 1=include pixel, but not its neighbors and 2=include pixel and its neighbors",'00000000000000022221100020022222'),ParValue("procbias","b","Process bias image data",True),ParRange("biasthresh","i","Minimum bias offset for invalid pixels in adu",6,3,4000),ParValue("evtsext","s","Event list extension",'EVENTS'),ParValue("writebias","b","Output faint+bias bias images",False),ParValue("outbias","f","Prefix for faint+bias bias images",None),ParValue("obsfile","f","Observation parameter file",None),ParValue("clobber","b","Overwrite if file exists",False),ParRange("verbose","i","Debug level",0,0,5)],
    }


parinfo['acis_classify_hotpix'] = {
    'istool': True,
    'req': [ParValue("infile","f","Input event filename",None),ParValue("outfile","f","Output hotpixel filename",None),ParValue("badpixfile","f","Input bad pixel filename",None),ParValue("hotpixfile","f","Input filename containing suspicious hot pixels",None)],
    'opt': [ParRange("expnothresh","i","Minimum difference in frame numbers between hot pixels",10,2,10000),ParValue("clobber","b","Overwrite if file exists",False),ParRange("verbose","i","Debug level",0,0,5)],
    }
...
parinfo['acis_find_hotpix'] = {
    'istool': True,
    'req': [ParValue("infile","f","Input event list",None),ParValue("outfile","f","Output event list",None),ParValue("badpixfile","f","acis bad pixel file",'NONE'),ParValue("maskfile","f","Name of input mask file ( <filename> | none | NONE )",'NONE')],
    'opt': [ParRange("probthresh","r","Minimum significance of potentially bad pixels after trials (e.g. 1 sig = 0.159, 2 sig = 0.0228, and 3 sig = 0.00135)",1.0e-3,1e-10,0.1),ParRange("regwidth","i","Size of reference region for comparison (e.g. 7 pixels x 7 pixels)",7,3,255),ParValue("clobber","b","OK to overwrite existing output file?",False),ParRange("verbose","i","Verbosity level",0,0,5)],
    }
...
parinfo['add_grating_orders'] = {
    'istool': True,
    'req': [ParValue("pha2","f","Input PHA2 spectrum",None),ParValue("order","s","Order of the grating spectra to extract and add together",'1'),ParSet("garm","s","Grating Arm",'MEG',["HEG","MEG","LEG"]),ParValue("garfm","f","Negative order grating ARF",None),ParValue("garfp","f","Positive order grating ARF",None),ParValue("root","f","Root name for output files",None)],
    'opt': [ParSet("gtype","s","Spectrum grouping type",'NONE',["NONE","BIN","SNR","NUM_BINS","NUM_CTS","ADAPTIVE","ADAPTIVE_SNR"]),ParValue("gspec","s","Spectrum grouping specs (NONE,10,etc)",'NONE'),ParValue("clobber","b","Clobber existing output files?",False),ParRange("verbose","i","Debug Level(0-5)",0,0,5)],
    }


parinfo['add_grating_spectra'] = {
    'istool': True,
    'req': [ParValue("pha1","f","First input PHA spectrum",None),ParValue("pha2","f","Second input PHA spectrum",None),ParValue("garf1","f","First input ARF",None),ParValue("garf2","f","Second input ARF",None),ParValue("root","f","Root name for output files",None)],
    'opt': [ParSet("gtype","s","Spectrum grouping type",'NONE',["NONE","BIN","SNR","NUM_BINS","NUM_CTS","ADAPTIVE","ADAPTIVE_SNR"]),ParValue("gspec","s","Spectrum grouping specs (NONE,10,etc)",'NONE'),ParValue("clobber","b","Clobber existing output files?",False),ParRange("verbose","i","Debug Level(0-5)",0,0,5)],
    }
...
parinfo['addresp'] = {
    'istool': True,
    'req': [ParValue("infile","f","RMF files",None),ParValue("arffile","f","ARF files",None),ParValue("outfile","f","merged RMF file",None),ParValue("outarf","f","merged ARF file",None)],
    'opt': [ParValue("lookupTab","f","lookup table",'${ASCDS_CALIB}/addresp_header_lookup.txt'),ParRange("thresh","r","low threshold of energy cut-off probability",1e-06,0,None),ParValue("clobber","b","overwrite existing output file (yes|no)?",True),ParRange("verbose","i","verbosity level (0 = no display)",0,0,5)],
    }
...
parinfo['aprates'] = {
    'istool': True,
    'req': [ParRange("n","i","Source counts",None,0,None),ParRange("A_s","r","Source area [arcsec]",None,0,None),ParRange("alpha","r","Source PSF fraction",None,0,None),ParRange("T_s","r","Source exposure time [sec]",None,0,None),ParRange("E_s","r","Source average exposure [cm^2 sec]",None,0,None),ParRange("eng_s","r","Average energy in src region [ergs]",None,0,None),ParRange("flux_s","r","Average flux in src region [ergs/cm^2/s]",None,0,None),ParRange("m","i","Background counts",None,0,None),ParRange("A_b","r","Background area [arcsec]",None,0,None),ParRange("beta","r","Background PSF fraction",None,0,None),ParRange("T_b","r","Background exposure time [sec]",None,0,None),ParRange("E_b","r","Background average exposure [cm^2 sec]",None,0,None),ParRange("eng_b","r","Average energy in bkg region [ergs]",None,0,None),ParRange("flux_b","r","Average flux in bkg region [ergs/cm^2/s]",None,0,None),ParRange("conf","r","Confidence region",0.68,0,1),ParValue("outfile","f","Output parameter file name",None)],
    'opt': [ParRange("resolution","r","Resolution of errors",0.01,0,1),ParSet("pdf","s","Method to estimate errors",'alternate',["gaussian","alternate"]),ParValue("max_counts","i","Maximum (rn-m)/(rf-g) before switching to Gaussian",50),ParValue("itermax","i","Maximum number of iterations to determine PDF grid",100),ParValue("nsigma","r","Number of sigma from S_mle to evaluate PDF",5.0),ParValue("pmin","r","Ratio of PDF peak to extreme",1.0e-5),ParRange("verbose","i","Tool verbosity",0,0,5)],
    }
...
parinfo['arfcorr'] = {
    ...
    'opt': [ParValue("energy","r","energy in keV (0 = read from ARF)",None),ParRange("e_step","i","energy stepsize (read every 'e_step' row from ARF)",50,1,None),ParRange("radlim","r","radius at which PSF artificially drops to 0 (multiplies last radius)",2.0,1,None),ParRange("nsubpix","i","subpixelation (1d) of ecf model pixels to choose image pix value",5,1,None),ParRange("nfracpix","i","subpixelation (1d) of ecf pixels when calculating psffrac",1,1,None),ParValue("ecffile","f","ecf file",'${CALDB}/data/chandra/default/reef/hrmaD1996-12-20reefN0001.fits'),ParValue("clobber","b","OK to overwrite existing output file?",False),ParRange("verbose","i","Debug level (0-5)",0,0,5)],
    }
...
parinfo['calquiz'] = {
    'istool': True,
    'req': [ParValue("infile","f","File to get meta data from",None),ParValue("telescope","s","telescope",None),ParValue("instrument","s","instrument",None),ParValue("product","s","What data product?",None),ParValue("calfile","s","caldb specifier",None)],
    'opt': [ParValue("outfile","s","File(s) found",None),ParValue("echo","b","Print keyword value to screen?",False),ParRange("verbose","i","verbosity level (0 = no display)",0,0,5)],
    }
...
parinfo['combine_spectra'] = {
    ...
    'opt': [ParValue("src_arfs","f","Source ARF files to combine; enter list or '@stack'",None),ParValue("src_rmfs","f","Source RMF files to combine; enter list or '@stack'",None),ParValue("bkg_spectra","f","Background PHA files to combine; enter list or '@stack'",None),ParValue("bkg_arfs","f","Background ARF files to combine; enter list or '@stack'",None),ParValue("bkg_rmfs","f","Background RMF files to combine; enter list or '@stack'",None),ParValue("clobber","b","OK to overwrite existing output file?",True),ParRange("verbose","i","Debug Level(0-5)",0,0,5)],
    }
...
parinfo['deflare'] = {
    ...
    'opt': [ParValue("nsigma","r","lc_clean 'clip', lc_sigma_clip 'sigma': no. of sigma about the mean to use to clip data",3),ParValue("plot","b","Plot light curve and histograms of values?",False),ParValue("save","f","PostScript file to save plot as?",None),ParSet("rateaxis","s","Should count-rate be on 'y' or 'x' axis of top plot?",'y',["x","y"]),ParSet("pattern","s","Pattern to use in plot for filled regions representing excluded time intervals",'solid',["nofill","solid","updiagonal","downdiagonal","horizontal","vertical","crisscross","brick","grid","hexagon","zigzag","wave"]),ParValue("good_color","s","Color to use in plot to draw 'good' data points",'green'),ParValue("exclude_color","s","Color to use in plot for excluded time intervals",'red'),ParValue("minlength","r","lc_sigma_clip: min. no. of consecutive time bins which must pass rate filter",3),ParValue("mean","r","lc_clean: mean counte rate (ct/s)",0),ParValue("stddev","r","lc_clean 'sigma': standard deviation of signal",0),ParValue("scale","r","lc_clean: scale factor about the mean rate",1.2),ParValue("minfrac","r","lc_clean: minimum fraction of good bins",0.1),ParRange("verbose","i","Debug level",1,0,5)],
    }
...
parinfo['dither_region'] = {
    'istool': True,
    'req': [ParValue("infile","f","Input aspect solution or histogram file(s)",None),ParValue("region","s","Region specification",None),ParValue("outfile","f","Output file name",None)],
...
parinfo['dmellipse'] = {
    ...
    'opt': [ParSet("shape","s","Shape of region",'ellipse',["ellipse","rotbox"]),ParValue("x_centroid","r","Default X-centroid to use (physical coords)",None),ParValue("y_centroid","r","Default Y-centroid to use (physical coords)",None),ParValue("angle","r","Default [aeg]",None),ParValue("ellipticity","r","Default ellipticity",None),ParValue("fix_centroid","b","Fix centroid?",False),ParValue("fix_angle","b","Fix angle?",False),ParValue("fix_ellipticity","b","Fix ellipticity?",False),ParRange("tolerance","r","Tolerance on fraction",0.001,0,1),ParRange("minstep","r","Minimum step size [pixels]",0.001,0,1),ParRange("maxwalk","i","Maximum number of equal-fraction walks",10,1,None),ParRange("step","r","Initial step size [pixels]",1,0,None),ParValue("normalize","b","Normalize by sum of pixels",True),ParValue("clobber","b","Remove outfile if it exists",False),ParRange("verbose","i","Tool chatter level",0,0,5)],
    }
...
parinfo['dmimglasso'] = {
    ...
    'opt': [ParSet("coord","s","Coordinate system of xpos,ypos",'physical',["physical","logical"]),ParSet("value","s","Hi/low values",'absolute',["absolute","percent","delta"]),ParValue("clobber","b","Clobber existing files",False),ParRange("verbose","i","Tool verbosity",0,0,0)],
    }
...
parinfo['fluximage'] = {
    'istool': True,
    'req': [ParValue("infile","s","Input events file",None),ParValue("outroot","f","Output root of data products?",None),ParRange("binsize","r","Image binning factor",8,0,None),ParValue("bands","s","Energy bands, comma-separated list, min:max:center in keV or ultrasoft, soft, medium, hard, broad, wide, CSC",'broad')],
    'opt': [ParValue("asolfile","f","Input aspect solutions",None),ParValue("badpixfile","f","Input bad pixel file",None),ParValue("maskfile","f","Input mask file",None),ParValue("pbkfile","f","Input pbk file for ACIS observations",None),ParValue("dtffile","f","Input dtf file for HRC observations",None),ParValue("normalize","b","Normalize exposure map?",False),ParValue("expmapthresh","s","Remove low-exposure regions? '2%' excludes pixels where exposure is < 2% of the maximum",'1.5%'),ParValue("parallel","b","Run processes in parallel?",True),ParValue("tmpdir","s","Directory for temporary files",'${ASCDS_WORK_PATH}'),ParValue("cleanup","b","Delete intermediary files?",True),ParValue("clobber","b","OK to overwrite existing output file?",False),ParRange("verbose","i","Verbosity level?",1,0,5)],
    }


parinfo['fullgarf'] = {
    'istool': True,
    'req': [ParValue("phafile","f","Input PHA file (Type I or II)",None),ParRange("pharow","i","Row in Type II PHA file (ignored if Type I)",1,0,99),ParValue("evtfile","f","Event file",None),ParValue("asol","f","Aspect offsets file",None),ParValue("engrid","f","Energy grid spec",None),ParValue("dtffile","s","Dead time correction factor; ACIS->evt file; HRC -> dtf file",')evtfile'),ParValue("badpix","f","Bad pixel file; (filename|NONE|CALDB)",None),ParValue("rootname","s","Output rootname",None),ParValue("pbkfile","s","NONE, or the name of the parameter block file",None),ParValue("maskfile","s","NONE, or name of ACIS window mask file",None)],
...
parinfo['lim_sens'] = {
    ...
    'opt': [ParValue("expfile","f","Exposure file",None),ParValue("rbkfile","f","Random background map name",None),ParValue("bscale","i","alpha * Es value",5),ParValue("snr_limit","i","Signal to Noise ratio",3),ParValue("const_r","b","Use a constant background ratio (bscale^2 - 1)?",False),ParValue("clobber","b","OK to overwrite existing output file(s)?",False),ParRange("verbose","i","Verbosity level",0,0,5)],
    }
...
parinfo['make_psf_asymmetry_region'] = {
    'istool': True,
    'req': [ParValue("infile","f","Input dataset",None),ParValue("outfile","f","Output region file name",None),ParRange("x","r","X coordinate of source (SKY)",None,0.5,65535.5),ParRange("y","r","Y coordinate of source (SKY)",None,0.5,65535.5)],
...
parinfo['merge_all'] = {
    'istool': True,
    'req': [ParValue("evtfile","f","Input event file(s)",None),ParValue("asol","f","Input asol file(s); time-sorted",None),ParValue("dtffile","f","Input Live Time Correction list files for HRC-I",None),ParValue("chip","s","ACIS CCD ID(s), comma-separated or range; or 'HRC-I' or 'HRC-S'",None),ParValue("refcoord","f","Reference coordinates or evt2 file for reproject_events",None),ParValue("xygrid","s","XY grid (e.g., 0.5:8192.5:#1024,0.5:8192.5:#1024)",None),ParValue("energy","f","Energy or spectral weights file for mkinstmap",None),ParValue("merged","f","Output merged event file",None),ParValue("expmap","f","Output merged exposure map",None),ParValue("expcorr","f","Output exposure-corrected image",None)],
    'opt': [ParValue("intdir","s","Directory for intermediate files",'/tmp'),ParValue("clobber","b","OK to overwrite existing output file(s)?",False)],
    }
...
parinfo['mkpsf'] = {
    'istool': True,
    'req': [ParSet("coord","s","input coordinate system",'SKY',["SKY","DET"]),ParRange("binspax","r","PSF binning in x direction",None,0.25,256.0),ParRange("binspay","r","PSF binning in y direction",None,0.25,256.0),ParRange("sizeoutx","i","PSF size in x direction",None,2,2048),ParRange("sizeouty","i","PSF size in y direction",None,2,2048),ParValue("infile","f","input file",None),ParRange("energy","r","energy in keV",None,0,None),ParValue("x","r","x",None),ParValue("y","r","y",None),ParValue("psflibfile","f","PSF library file",'CALDB'),ParValue("outfile","f","output file",None),ParValue("outpsffile","f","psflib data output basename (\'.\' to use output file)",None)],
    'opt': [ParValue("geompar","f","Parameter file for Pixlib Geometry files",'geom'),ParRange("rotpts","i","number of pixel points in x or y direction for rotation",9,1,None),ParRange("verbose","i","verbose mode",0,0,5),ParValue("clobber","b","overwrite existing output file?",False)],
    }


parinfo['mkpsfmap'] = {
    'istool': True,
    'req': [ParValue("infile","f","Input image file name",None),ParValue("outfile","f","Output image file name",None),ParValue("energy","r","Energy of PSF to lookup [keV]",None),ParRange("ecf","r","ECF of PSF to lookup",None,0,1)],
...
parinfo['modelflux'] = {
    ...
    'opt': [ParValue("oemin","r","Energy range lower bound for rate (keV), default=emin",None),ParValue("oemax","r","Energy range upper bound for rate (keV), default=emax",None),ParValue("rate","r","count rate in counts s^-1, default=1.0",1.0),ParValue("pflux","r","photon flux in energy range in photon cm^-2 s^-1",None),ParValue("flux","r","energy flux in energy range in erg cm^-2 s^-1",None),ParSet("opt","s","input type: (use this to determine the others)",'rate',["rate","flux","pflux"]),ParRange("verbose","i","verbosity setting",3,0,5)],
    }
...
parinfo['psextract'] = {
    'istool': True,
    'req': [ParValue("events","f","Source events specification",None),ParValue("bgevents","f","Background events specification",None),ParValue("root","s","Root name for output files",None),ParValue("asol","f","Source aspect solution file",None),ParValue("bgasol","f","Background aspect solution file",None),ParValue("pbkfile","s","NONE, or name of the parameter block file",None)],
    'opt': [ParValue("dafile","s","CALDB, NONE, or name of ACIS dead-area calibration file",'CALDB'),ParSet("ptype","s","pi or pha",'pi',["pi","pha"]),ParSet("gtype","s","Spectrum grouping type (same as grouptype in dmgroup)",'NONE',["NONE","BIN","SNR","NUM_BINS","NUM_CTS","ADAPTIVE","ADAPTIVE_SNR","BIN_WIDTH","MIN_SLOPE","MAX_SLOPE","BIN_FILE"]),ParValue("gspec","s","Spectrum grouping specs (NONE,1:1024:10,etc)",'NONE'),ParValue("clobber","b","OK to overwrite existing output file?",False),ParRange("verbose","i","Debug Level(0-5)",0,0,5)],
    }
...
parinfo['reproject_image_grid'] = {
    'istool': True,
    'req': [ParValue("infile","f","Input image file name",None),ParValue("outfile","f","Output file name",None),ParRange("xsize","i","X-Size of output image in image pixels",1,1,None),ParRange("ysize","i","Y-Size of output image in image pixels",1,1,None),ParValue("xcenter","s","X-Center of image in cood_sys",None),ParValue("ycenter","s","Y-Center of image in cood_sys",None),ParRange("theta","r","Angle between world coord north and output y axis",0,0,360),ParValue("pixelsize","s","Pixel size",None)],
    'opt': [ParRange("resolution","i","Number of point per side to evaluate",1,0,None),ParSet("method","s","Average value",'sum',["sum","average"]),ParSet("coord_sys","s","Coordinate system to match images in",'world',["world","logical","physical"]),ParValue("lookupTab","f","lookup table",'${ASCDS_CALIB}/dmmerge_header_lookup.txt'),ParValue("clobber","b","Clobber existing files",False),ParRange("verbose","i","Tool verbosity",0,0,5)],
    }
...
parinfo['sky2tdet'] = {
    ...
    'opt': [ParValue("geompar","s","Pixlib geometry file",'geom'),ParRange("verbose","i","Verbosity",0,0,5),ParValue("clobber","b","Remove existing files?",False)],
    }
...
parinfo['specextract'] = {
    'istool': True,
    'req': [ParValue("infile","f","Source event file(s)",None),ParValue("outroot","f","Output directory path + root name for output files",None),ParValue("weight","b","Should response files be weighted?",True),ParValue("correct","b","Apply point source aperture correction to ARF?",False),ParValue("bkgfile","f","Background event file(s)",None),ParValue("bkgresp","b","Create background ARF and RMF?",True),ParValue("asp","f","Source aspect solution or histogram file(s)",None),ParValue("combine","b","Combine output spectra and responses?",False),ParValue("pbkfile","f","pbkfile input to mkwarf",None),ParValue("mskfile","f","mskfile input to mkwarf",None)],
    'opt': [ParValue("rmffile","f","rmffile input for CALDB",'CALDB'),ParSet("ptype","s","PI or PHA",'PI',["PI","PHA"]),ParSet("grouptype","s","Spectrum grouping type (same as grouptype in dmgroup)",'NUM_CTS',["NONE","BIN","SNR","NUM_BINS","NUM_CTS","ADAPTIVE","ADAPTIVE_SNR","BIN_WIDTH","MIN_SLOPE","MAX_SLOPE","BIN_FILE"]),ParValue("binspec","s","Spectrum grouping specification (NONE,1:1024:10,etc)",'15'),ParSet("bkg_grouptype","s","Background spectrum grouping type (NONE, BIN, SNR, NUM_BINS, NUM_CTS, or ADAPTIVE)",'NONE',["NONE","BIN","SNR","NUM_BINS","NUM_CTS","ADAPTIVE"]),ParValue("bkg_binspec","s","Background spectrum grouping specification (NONE,10,etc)",None),ParValue("energy","s","Energy grid",'0.3:11.0:0.01'),ParValue("channel","s","RMF binning attributes",'1:1024:1'),ParValue("energy_wmap","s","Energy range for (dmextract) WMAP input to mkacisrmf",'300:2000'),ParValue("binwmap","s","Binning factor for (dmextract) WMAP input to mkacisrmf",'tdet=8'),ParValue("dafile","f","dafile input to mkwarf",'CALDB'),ParValue("badpixfile","f","Bad pixel file for the observation",None),ParValue("clobber","b","OK to overwrite existing output file?",False),ParRange("verbose","i","Debug Level(0-5)",1,0,5)],
    }
...
parinfo['tgdetect'] = {
    ...
    'opt': [ParValue("temproot","s","Path and root file name to be given to temporary files",None),ParValue("keeptemp","b","Keep temporary files?",False),ParValue("keepexit","b","Keep exit status file?",False),ParValue("zo_pos_x","s","Center GZO filter sky X position (default=pixel(ra_nom))",'default'),ParValue("zo_pos_y","s","Center GZO filter sky Y position (default=pixel(dec_nom))",'default'),ParValue("zo_sz_filt_x","s","Size of GZO filter in X pixels (ACIS=400; HRC=1800)",'default'),ParValue("zo_sz_filt_y","s","Size of GZO filter in Y pixels (ACIS=400; HRC=1800)",'default'),ParRange("snr_thresh","r","SNR threshold to select the detected sources",40,3,10000),ParValue("expstk","f","list of exposure map files",'none'),ParRange("thresh","r","celldetect source threshold",')celldetect.thresh',3,10000),ParValue("ellsigma","r","Size of output source ellipses (in sigmas)",3.0),ParValue("expratio","r","cutoff ratio for source cell exposure variation",0),ParValue("findpeaks","b","find local peaks for celldetect",True),ParValue("celldetect_log","f","make a celldetect log file?",')celldetect.log'),ParValue("psftable","f","table of PSF size data, for celldetect",')celldetect.psftable'),ParRange("fixedcell","i","celldetect fixed cell size to use",15,0,999),ParRange("fixedcell_cc_mode","i","celldetect fixed cell size to use for CC mode ACIS data",15,0,999),ParValue("bkgfile","f","background file, for celldetect",'none'),ParRange("bkgvalue","r","background count/pixel, for celldetect",')celldetect.bkgvalue',0,None),ParRange("bkgerrvalue","r","background error, for celldetect",')celldetect.bkgerrvalue',0,None),ParRange("eband","r","energy band, for celldetect",')celldetect.eband',0,None),ParRange("eenergy","r","encircled energy of PSF, for celldetect",')celldetect.eenergy',0,None),ParValue("snrfile","f","celldetect snr output file (for convolution only)",'none'),ParValue("convolve","b","use convolutions for celldetect",')celldetect.convolve'),ParRange("xoffset","i","celldetect offset of x axis from optical axis",None,"INDEF","INDEF"),ParRange("yoffset","i","celldetect offset of y axis from optical axis",None,"INDEF","INDEF"),ParValue("cellfile","f","output cell size image file",'none'),ParValue("centroid","b","compute source centroids in celldetection?",True),ParRange("snr_ratio_limit","r","Value of SNR ratio to use as lower limit",')tgidselectsrc.snr_ratio_limit',0.0,1.0),ParValue("setsrcid","b","Set src ids in output file?",')tgidselectsrc.setsrcid'),ParRange("max_separation","r","Maximum allowed separation (arcsec) for sources to match",')tgmatchsrc.max_separation',0,20),ParValue("clobber","b","OK to overwrite existing output file(s)?",False),ParRange("verbose","i","Verbosity level (0 = no display)",0,0,5)],
    }
...
parinfo['wcs_match'] = {
    ...
    'opt': [ParValue("wcsfile","f","Input file with WCS used in transform",None),ParValue("logfile","f","debug log file ( STDOUT | stdout | <filename>)",'STDOUT'),ParRange("radius","r","radius used to match sources (arcsec)",12,0,None),ParRange("residlim","r","src pairs with residuals > residlim are dropped (arcsec)",2,0,None),ParRange("residfac","r","src pairs with residuals > residfac * position error are dropped",0,0,None),ParRange("residtype","i","residfac applies to: (0) each residual, (1) avg residuals",0,0,1),ParValue("clobber","b","Overwrite existing output dataset with same name?",False),ParRange("verbose","i","debug level (0-5)",0,0,5)],
    }
